...
numberOfResults = 7 # Select number of contexts to input
max_tokens: int = 4096
temperature: float = 0.3 #The amount of randomness injected into the response.
top_k: int = 50 # Only sample from the top K options for each subsequent token.
top_p: float = 0.0
...
## Some basic config
st.header("PETROVIETNAM GenAI")
st.markdown(
        "Xin chào, tôi là ứng dụng Generative AI của PetroVietnam. Tôi đã đọc qua các tài liệu về quy định mua sắm, các nghị định cũng như tìm hiểu các quy định qua các trang web Pháp luật Việt Nam. Hãy hỏi tôi bất kỳ nội dung nào liên quan, nếu tôi biết, tôi sẽ tìm kiếm và trả lời giúp bạn."
)
